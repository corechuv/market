...
export default function OrderDetails() {
  ...
  const [recoProducts, setRecoProducts] = React.useState<Product[]>([]);
  const [, setRecoLoading] = React.useState(false);
  // const [recoError, setRecoError] = React.useState<string|null>(null); // опционально

  React.useEffect(() => {
    let cancelled = false;
    setRecoLoading(true);
    getProducts({ q: "", sort: "name" })            // "popular" у API нет — вернёт "name"
      .then((arr) => { if (!cancelled) setRecoProducts(Array.isArray(arr) ? arr : []); })
      .catch(() => { if (!cancelled) setRecoProducts([]); })
  ...
  return (
    <PageLayout title="Order" onBack={() => navigate(backTo)}>
      <div style={{ display: "flex", gap: 40, flexDirection: "column" }}>
        <div className={styles.stack}>
          ...
          <section>
            ...
            <div style={{display: "flex", flexDirection: "column", gap: 16}}>
              <Accordion title="00340434161094000001" margin={false}>
              ...
              <Accordion title="00340434161094000002" margin={false}>
                <OrderTrackingCompact
                  trackingNumber="00340434161094000002"
                  carrierName="DHL"
                  currentStatus="PICKUP_READY"
                  paidAt="2025-09-21T09:10:00Z"
                  lastUpdate="2025-09-21T14:32:00Z"
              ...
              <Accordion title="00340434161094000003" margin={false}>
                <OrderTrackingCompact
                  trackingNumber="00340434161094000003"
                  carrierName="DHL"
                  currentStatus="PICKUP_READY"
                  paidAt="2025-09-21T09:10:00Z"
                  lastUpdate="2025-09-21T14:32:00Z"
          ...
          <section>
            ...
            <ul className="summary__list">
              <li>
                <span>Method</span>
                <span>{order.payment ? paymentMethodLabel(order.payment.method) : "—"}</span>
              </li>
              <li>
                <span>Paid</span>
                <span>
                  {order.payment
                    ? `${fmtMoney(order.payment.amountCents / 100, account.settings.currency, locale)}${order.payment.paidAt ? " • " + new Date(order.payment.paidAt).toLocaleString(locale) : ""
                    }`
                    : "—"}
                </span>
              </li>
        ...
        <ProductCarouselRich label="Consider these items" products={recoProducts}
          visibleItems={4}
          onItemClick={(p) => nav(`/product/${p.id}`)}
        // или, если хотите <a href> вместо onClick:
        // itemLinkBuilder={(p) => `/product/${p.id}`}
        />
        <ProductCarouselRich label="These are for you" products={recoProducts}
          visibleItems={4}
          onItemClick={(p) => nav(`/product/${p.id}`)}
        // или, если хотите <a href> вместо onClick:
        // itemLinkBuilder={(p) => `/product/${p.id}`}
