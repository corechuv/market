...
function buildIndexes(source: Category[]): CategoryIndexes {
  ...
  for (const [id, c] of byId) byId.set(id, { ...c, fullSlug: computeFullSlug(c) });
...
// ---- крошечный кэш в sessionStorage (опциональный) ----
...
function saveCache(tree: Category[]) {
  try {
    sessionStorage.setItem(CACHE_KEY, JSON.stringify({ ts: Date.now(), data: tree }));
  } catch { }
}
...
export async function syncFromApi(): Promise<void> {
  ...
  try {
    ...
    const url = `${API_BASE}/categories/tree?depth=3&activeOnly=true`;
    ...
    // пробуем стартовать с кэша, если он есть
    ...
    if (cached) {
      ...
      window.dispatchEvent(new CustomEvent("categories:error", { detail: _lastError }));
    }
  }
}
...
export function getCategoryById(id: string) { return ensureIndex().byId.get(id); }
export function getCategoryByFullSlug(fullSlug: string) { return ensureIndex().byFullSlug.get(fullSlug); }
export function findCategoriesBySlug(slug: string) { return ensureIndex().list.filter((c) => c.slug === slug); }
...
// автоинициализация (ленивая: сначала кэш, затем API)
(() => {
  const cached = loadCache();
  if (cached) {
    INDEX = buildIndexes(cached);
    _loaded = true;
    // не шлём событие тут, чтобы не мигал интерфейс до syncFromApi
  }
  // не блокируем — дергается из компонентов/роутера
})();
